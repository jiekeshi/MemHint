...
# Strict: return (var);  return var;
_RETURN_ONLY_VAR_RE = re.compile(
    r"^\s*return\s*\(*\s*(?P<var>[A-Za-z_]\w*)\s*\)*\s*;?\s*$"
)
...
class WarningValidator:
    ...
    def __init__(
        ...
        # Minimal, conservative ownership-transfer sinks that commonly "consume" a heap object.
        # This is project-dependent; keep small to avoid false negatives.
        self.ownership_sinks: set[str] = {
            # tmux-ish patterns / queueing / buffers
            "paste_set",
            "cmdq_get_callback",
            "cmdq_append",
            # containers (if you treat insertion as ownership transfer)
            "RB_INSERT",
            "TAILQ_INSERT_HEAD",
            "TAILQ_INSERT_TAIL",
            "TAILQ_INSERT_AFTER",
    ...
    def _check_feasibility(
        ...
        try:
            ...
            if warning.issue_type in (MemoryIssueType.MEMORY_LEAK, MemoryIssueType.MEMORY_LEAK_FILTERED):
    ...
    def _return_is_exact_var(self, code: str, var: str) -> bool:
        """True iff the statement is exactly 'return var;' (optionally parenthesized)."""
        if not code or not var:
            return False
        m = _RETURN_ONLY_VAR_RE.match(code.strip())
        return bool(m and m.group("var") == var)

    def _is_ownership_transfer_call(self, code: str, alloc_var: str) -> bool:
        """Heuristic: treat some calls as ownership transfer if they mention alloc_var."""
        if not code or not alloc_var:
            return False
        # Must mention alloc_var as a token, and contain a known sink callee.
        if not self._mentions_var(code, alloc_var):
            return False
        for fn in self.ownership_sinks:
            if re.search(rf"\b{re.escape(fn)}\s*\(", code):
                return True
        return False

    def _collect_aliases_from_trace(
        self,
        cfg: dict[int, CFGNode],
        trace_lines: list[int],
        alloc_var: str,
    ) -> set[str]:
        """Collect trivial aliases from trace lines only (cheap, reduces FPs).
        ...
        trace_set = set(trace_lines or [])
        for n in cfg.values():
            if n.node_type != NodeType.ASSIGN:
                continue
            if n.line not in trace_set:
                continue

            m = _ASSIGN_RE.match((n.code or "").strip())
            if not m:
                continue
            ...
            # Only keep simple identifier aliases (avoid fields, *out, etc.)
            if not re.fullmatch(r"[A-Za-z_]\w*", lhs):
                continue

            # tmp = alloc_var;
            if self._mentions_var(rhs, alloc_var):
                aliases.add(lhs)
    ...
    def _check_leak_feasibility(
        ...
        alloc_nodes = [n for n in cfg.values() if n.node_type == NodeType.ALLOC]
        # Prefer RETURN as leak sinks; EXIT(0) node is artificial and can create surprising models.
        exit_nodes = [n for n in cfg.values() if n.node_type == NodeType.RETURN]
        if not alloc_nodes:
        ...
        # Cheap, trace-scoped aliasing: allow free(tmp) when tmp = alloc_var appears in trace.
        alias_vars = self._collect_aliases_from_trace(cfg, trace_lines, alloc_var)
        alias_vars.add(alloc_var)

        def alloc_matches_var(node: CFGNode) -> bool:
            # Accept either node.variable or node.obj as the allocation identity.
            # This helps when the LHS is not a plain identifier.
            if node.node_type != NodeType.ALLOC:
                return False
            ident = (node.variable or node.obj or "").strip()
            return ident == alloc_var

        def free_matches_alloc(node: CFGNode) -> bool:
            if node.node_type != NodeType.FREE:
                return False

            # Strong match: free(alloc_var) OR free(alias) where alias is tmp=alloc_var from trace
            if node.variable and node.variable in alias_vars:
                return True

            o = (node.obj or "").strip()
            if o and (o == alloc_var or o in alias_vars):
                return True
        ...
        for node in cfg.values():
            ...
            if node.node_type == NodeType.RETURN and self._return_is_exact_var(node.code, alloc_var):
                escape_now = True
            ...
            # Escape: ownership transfer via call (CFGBuilder models many calls as DEREF nodes)
            if node.node_type == NodeType.DEREF and self._is_ownership_transfer_call(node.code, alloc_var):
                escape_now = True
