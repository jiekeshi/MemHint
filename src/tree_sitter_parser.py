...
class CodeParser:
    ...
    def get_function_containing_line(
        self,
        file_path: Path,
        line_1based: int,
        preprocess: bool = False,
    ) -> Optional[FunctionInfo]:
        """Return the function that contains the given line (1-based).
        Prefer the innermost function if several contain the line (e.g. nested).
        Returns None if no function contains the line or on parse error.
        """
        try:
            if preprocess:
                source = self._preprocess_file(file_path)
        ...
        candidates: list[tuple[int, int, object]] = []  # (span, start_line, node)
        ...
        for node in self._iter_nodes(tree.root_node):
            ...
            if start_line <= line_1based <= end_line:
                span = end_line - start_line
                candidates.append((span, start_line, node))

        if not candidates:
            return None
        # Smallest span = innermost containing function
        candidates.sort(key=lambda x: (x[0], x[1]))
        _, _, node = candidates[0]
        return self._extract_function(node, source, file_path_str)
